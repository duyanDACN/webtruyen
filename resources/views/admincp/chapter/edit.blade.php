...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">Edit chapter</div>
                @if ($errors->any())
                ...
                <div class="card-body">
                    ...
                    <form method="post" action="{{route('chapter.update',['chapter'=>$chapter->chapter_id])}}">
                        @csrf
                        @method('put')
                        <div class="mb-3">
                          <label for="exampleInputEmail1" class="form-label">Tên chapter</label>
                          <input type="text" value="{{$chapter->ten_chapter}}" name="tenchapter" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
                        </div>
                        <div class="mb-3">
                          <label for="exampleInputPassword1" class="form-label">Tóm tắt chapter</label>
                          <textarea name="tomtatchapter" class="form-control" id="" cols="30" rows="10">{{$chapter->tomtat_chapter}}</textarea>
                        </div>
                        <div class="mb-3">
                            <label for="exampleInputPassword1" class="form-label">Nội dung chapter</label>
                            <textarea name="noidungchapter" class="form-control" id="" cols="30" rows="10">{{$chapter->noidung_chapter}}</textarea>
                          </div>
